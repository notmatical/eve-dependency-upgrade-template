...
export default defineEval({
  ...
  async test(t) {
    ...
    t.judge.autoevals
      .closedQA(
        "Is the comment free of padding, with no line counts or file sizes the diff already shows, no narration of the agent's own process or ordering, and no softeners such as 'I went ahead and' or 'successfully'?",
        { on: t.reply ?? "(no reply)" },
      )
      .soft(0.5);
