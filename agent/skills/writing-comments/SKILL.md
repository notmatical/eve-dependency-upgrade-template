...
## How these two comments reach the thread

They travel differently, and confusing them is why both go wrong. The **attempt marker** is an
explicit `github__addPullRequestComment` call you make before starting. The **result** is your final
message of the run, which the channel publishes verbatim as a comment.

That second one matters more than it sounds: your closing message is not a reply to anybody. Nobody
reads the run that produced it. Opening with `Pushed successfully. Now the final comment:` publishes
exactly that, and it is the tell that an agent wrote the thread rather than a colleague.
...
seen. Those differ more often than you would expect, since a package documented as breaking one way can
