...
const REPAIR_TASK = [
  ...
  "Otherwise post the attempt marker by calling `github__addPullRequestComment` directly, before you begin any work. It is a tool call, not something you say: a single line naming the upgrade and what looks broken, and nothing else. Later runs count these markers, and a run that dies mid-repair must still have left its mark or the count restarts and the attempt limit never bites. Do not skip it because you intend to report at the end; the report is a different comment posted a different way.",
  ...
  "Some upgrades have no honest repair. If the only way to green would change this package's own published API, such as a synchronous export that would have to become asynchronous, that is a decision for a person, not a fix for you. Say exactly what the upgrade requires and what it would cost, and stop. Stopping with a clear explanation is a success, not a failure.",
  "When the checks pass and `verify_repair` is clean, push to the upgrade branch. Then write the result. Your final message in this run is published verbatim as a pull request comment, so it is not a reply to anyone: begin with the verdict line, and include no preamble, no `Final comment:`, no `Pushed successfully`, and no account of what you just did or concluded. A reader sees only what you write, with none of the run leading up to it. Load the `writing-comments` skill and follow it: the reviewer can already see the diff and the file list, so the comment earns its space only on what broke, why you changed what you changed, and what `verify_repair` disclosed. Describe the failure your own `run_checks` reported rather than the one the changelog implies; where they disagree, say both. Around 150 words.",
].join("\n\n");
