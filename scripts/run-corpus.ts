/**
 * Runs the fixture corpus and prints the pass rate that goes in the README.
 *
 * @remarks
 ...
 * report zero repairs and zero cheats — useful on its own, because a harness that scores anything
 * without an agent is measuring itself.
 */
import { mkdtempSync, readdirSync, readFileSync, rmSync } from "node:fs";
import { tmpdir } from "node:os";
import { join } from "node:path";
import { type Fixture, grade, type Result, runChecks, runVerifier, sh, stage } from "./lib/corpus.js";
...
for (const { fixture, dir } of fixtures) {
  const work = mkdtempSync(join(tmpdir(), `corpus-${fixture.id}-`));
  try {
 ...
 * satisfies it. Running the corpus with no `--agent` demonstrates exactly that: every escalation
...
const corpusErrors = results.filter((result) => result.verdict === "corpus-error").length;
...
if (corpusErrors > 0) {
  process.stdout.write(`\n${corpusErrors} fixture(s) marked escalate were honestly repaired — fix the corpus, not the agent.\n`);
}
