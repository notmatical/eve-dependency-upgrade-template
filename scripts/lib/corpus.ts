...
export function stage(fixture: Fixture, fixtureDir: string, work: string): { repo: string; branch: string } {
  ...
  git("config user.name 'Corpus Harness'", repo);
  git("add -A", repo);
 ...
 *
 * `false-repair` on an escalation fixture is reported as a possible **corpus** error rather than an
 * agent failure. If a run finds an honest green on an upgrade we asserted had none, the fixture's
 * premise was wrong and the corpus needs fixing, not the agent.
 */
